<?xml version="1.0" encoding="UTF-8"?>
<!-- This file is generated by Kaoto DataMapper. Do not edit. -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0" xmlns:fn="http://www.w3.org/2005/xpath-functions">
    <xsl:output method="text" indent="yes"/>
    <xsl:param name="discord"/>
    <xsl:variable name="discord-x" select="json-to-xml($discord)"/>
    <xsl:variable name="mapped-xml">
        <map xmlns="http://www.w3.org/2005/xpath-functions">
            <string key="timestamp">
                <xsl:value-of select="format-dateTime(current-dateTime(),'[Y0001]-[M01]-[D01]T[H01]:[m01]:[s01].[f001]Z')"/>
            </string>
            <string key="source">
                <xsl:value-of select="&quot;discord&quot;"/>
            </string>
            <string key="user">
                <xsl:value-of select="$discord-x/fn:map/fn:map[@key='d']/fn:map[@key='author']/fn:string[@key='global_name']"/>
            </string>
            <string key="text">
                <xsl:value-of select="$discord-x/fn:map/fn:map[@key='d']/fn:string[@key='content']"/>
            </string>
        </map>
    </xsl:variable>
    <xsl:template match="/">
        <xsl:value-of select="xml-to-json($mapped-xml)"/>
    </xsl:template>
</xsl:stylesheet>